...
class TensorWrapper(object):
    def __init__(self, tensor, node_desc, graph_creator, activation_size=None, parameter_size=0.0):
        self.tensor = tensor
        global object_id
        ...
        if i < len(graph_creator.summary) and node_desc == str(graph_creator.summary[i]['layer_name']):
            ...
            self._node = graph.Node("node%d" % object_id, node_desc=str(node_desc).replace('\n', ''),
            ...
            if activation_size is None:
                activation_size = 0.0
            self._node = graph.Node("node%d" % object_id, node_desc=str(node_desc).replace('\n', ''),
                                    activation_size=activation_size, parameter_size=parameter_size)
    ...
    def reshape(self, *wrapped_sizes):
        ...
        if len(sizes) == 1:
            wrapped_result = TensorWrapper(result, "Reshape", self.graph_creator,
                                           activation_size=self.node().activation_size)
        else:
            wrapped_result = TensorWrapper(result,
                                           "Reshape(%s)" % ", ".join([str(size) for size in sizes[1:]]),
                                           self.graph_creator,
                                           activation_size=self.node().activation_size)
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        for in_edge in in_edges:
    ...
    def expand(self, *wrapped_sizes):
        ...
        if len(sizes) == 1:
            wrapped_result = TensorWrapper(result, "Expand", self.graph_creator,
                                           activation_size=4 * result.numel())
        else:
            wrapped_result = TensorWrapper(result,
                                           "Expand(%s)" % ", ".join([str(size) for size in sizes[1:]]),
                                           self.graph_creator,
                                           activation_size=4 * result.numel())
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        for in_edge in in_edges:
    ...
    def __add__(self, other):
        ...
        wrapped_result = TensorWrapper(result_tensor, "Add", self.graph_creator,
                                       activation_size=4 * result_tensor.numel())
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        if isinstance(other, TensorWrapper):
...
class GraphCreator(object):
    ...
    def hook_modules(self, module, root=False):
        ...
        def forward_wrapper(self, *wrapped_inputs, **kwargs):
            input = []
            ...
            for i in range(len(wrapped_inputs_list)):
                if isinstance(wrapped_inputs_list[i], TensorWrapper):
                    ...
                    input.append(wrapped_inputs_list[i].tensor)
            result = this_creator.forward_original_methods[self](*input, **kwargs)
