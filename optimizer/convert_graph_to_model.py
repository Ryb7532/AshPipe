...
# This code includes modifications made by Ryb7532.
...
def convert_subgraph_to_module(graph, full_graph, num_subgraphs, module_name, initialize_weights,
    ...
    for node in nodes:
        ...
        return seq_len.int()""" % layer_name)
                ...
                elif declaration == "hidden":
                    m = re.search(r'hidden\((.*)\)', node.node_desc)
                    if m is None:
                        layer_declaration = "torch.nn.Parameter()\t\t##### hidden parameter #####"
                    else:
                        layer_declaration = "torch.nn.Parameter(%s)" % m.group(1)
                elif declaration == "MultiheadAttention":
                    node_desc = node.node_desc
                    module_arg = node_desc.split("  ")[1]
                    layer_declaration = "torch.nn.MultiheadAttention(%s)" % module_arg
                elif declaration == "StochasticDepth":
                    m = re.search(r'StochasticDepth\(p=(\d+.?\d+), mode=(.*)\)', node.node_desc)
                    prob = float(m.group(1))
                    mode = m.group(2)
                    layer_declaration = "StochasticDepth(%f, \"%s\")" % (prob, mode)
                    import_statements.append("from torchvision.ops import StochasticDepth")
                break
        ...
        if len(in_edges) == 0 and node.node_desc.startswith("Input"):
            ...
            if node.node_desc.startswith("Size"):
                ...
                if m is None:
                    layer_call = "%s = %s.size()\t\t##### Note: [] is needed because the type of output is `tuple` #####" % (output_name,
                ...
                for i in range(len(in_edges)):
                    if in_edges[i].node_desc.startswith("Size") or in_edges[i].node_desc.startswith("__getitem__"):
                ...
                if not m:
                    ...
                    layer_call = "%s = %s.view(%s, %s)" % (output_name,
                                                           ...
                                                           ", ".join(m))
            elif node.node_desc.startswith("Reshape"):
                size_node_ids = []
                input_node_id = None
                for i in range(len(in_edges)):
                    if in_edges[i].node_desc.startswith("Size") or in_edges[i].node_desc.startswith("__getitem__"):
                ...
                if not m:
                    size_output_names = [output_names[size_node_id] for size_node_id in size_node_ids]
                    layer_call = "%s = %s.reshape(%s)" % (output_name,
                                                          output_names[input_node_id],
                                                          ", ".join(size_output_names))
                elif not size_node_ids:
                    layer_call = "%s = %s.reshape(%s)" % (output_name,
                    ...
                    layer_call = "%s = %s.reshape(%s, %s)" % (output_name,
                                                              output_names[input_node_id],
                                                              output_names[size_node_id],
                                                              ", ".join(m))
                ...
                if m is None:
                    if "hidden" in in_edges[0].node_desc:
                        layer_call = "%s = None\t\t##### hidden[]: write args #####" % output_name
                    else:
                        layer_call = "%s = %s[]\t\t##### write args #####" % (output_name,
                                                    output_names[in_edges[0].node_id])
                else:
                    args_str = m.group(1)
                    if "hidden" in in_edges[0].node_desc:
                        layer_call = "%s = None\t\t##### hidden[%s] #####" % (output_name, args_str)
                    else:
                        layer_call = "%s = %s[%s]" % (output_name,
                                                      output_names[in_edges[0].node_id],
                                                      args_str)
                ...
                assert(len(in_edges) == 1)
                node1 = in_edges[0]
                layer_call = "%s = %s.transpose(%s)" % (output_name, output_names[node1.node_id], args_str)
            elif node.node_desc.startswith("Permute"):
                m = re.search(r'Permute\((.+)\)', node.node_desc)
                args_str = m.group(1)
                assert(len(in_edges) == 1)
                node1 = in_edges[0]
                layer_call = "%s = %s.permute(%s)" % (output_name, output_names[node1.node_id], args_str)
            elif node.node_desc == "self.get_seq_lens":
                assert(len(in_edges) == 1)
                in_node = in_edges[0]
                layer_call = "%s = %s(%s)" % (output_name, node.node_desc, output_names[in_node.node_id])
            elif node.node_desc.startswith("Expand"):
                size_node_ids = []
                input_node_id = None
                for i in range(len(in_edges)):
                    if in_edges[i].node_desc.startswith("Size") or in_edges[i].node_desc.startswith("__getitem__"):
                ...
                if not m:
                    size_output_names = [output_names[size_node_id] for size_node_id in size_node_ids]
                    layer_call = "%s = %s.expand(%s)" % (output_name,
                                                         output_names[input_node_id],
                                                         ", ".join(size_output_names))
                elif not size_node_ids:
                    layer_call = "%s = %s.expand(%s)" % (output_name,
                    ...
                    layer_call = "%s = %s.expand(%s, %s)" % (output_name,
                                                             output_names[input_node_id],
                                                             output_names[size_node_id],
                                                             ", ".join(m))
            elif node.node_desc.startswith("hidden"):
                layer_call = "%s = %s" % (output_name, layer_name)
            elif node.node_desc.startswith("MultiheadAttention"):
                layer_call = "%s = %s(%s, need_weights=False)\t\t##### Note: [] is needed because the type of output is `tuple` #####" % (
...
if __name__ == '__main__':
    ...
    if args.stage_to_num_ranks_map is not None:
        ...
        if pytorch_modules is not None:
            assert(len(stage_to_num_ranks_map) == len(pytorch_modules))
        else:
            assert(len(stage_to_num_ranks_map) == 1)
        num_modules = len(stage_to_num_ranks_map) + 1  # Add 1 for criterion.
    ...
    all_template_args.append({
        ...
        "stage_to_depth_map": str({str(i): (num_modules-2-i) for i in range(num_modules-1)}).replace("'", "\""),
        "recompute_stage": str([])
    })
    if args.stage_to_num_ranks_map is not None:
        stage_to_rank_map = {}
